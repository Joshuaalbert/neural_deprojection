...
class DecoderResBlock3D(AbstractModule):
    def __init__(self, out_size, post_gain, name=None):
        ...
        self.conv_block = snt.Sequential([tf.nn.relu, snt.Conv3D(hidden_size, 1,  padding=same_padding,name='conv_1'),
                                          tf.nn.relu, snt.Conv3D(hidden_size, 3,  padding=same_padding,name='conv_2'),
                                          tf.nn.relu, snt.Conv3D(hidden_size, 3,  padding=same_padding,name='conv_3'),
                                          tf.nn.relu, snt.Conv3D(out_size, 3, padding=same_padding, name='conv_4')])
...
def upsample3d(x):
    ...
    Returns:
        [batch, 2*W, 2*H, 2*D, C]
    """
    return tf.repeat(tf.repeat(tf.repeat(x,2,axis=1),2,axis=2), 2, axis=3)
    ...
    def __init__(self, hidden_size, num_channels=1, name=None):
        ...
        def _single_group(group_idx):
            ...
            res_blocks = [DecoderResBlock3D(blk_hidden_size, post_gain, name=f'blk_{res_blk}')
                          for res_blk in range(num_blk_per_group)]
            if group_idx < num_groups - 1:
                res_blocks.append(upsample3d)
            return snt.Sequential(res_blocks, name=f'group_{group_idx}')

        groups = [snt.Conv3D(hidden_size//2, 1,  padding=same_padding,name='input_group')]
        for groud_idx in range(num_groups):
            groups.append(_single_group(groud_idx))
        groups.append(
            snt.Sequential([tf.nn.relu, snt.Conv3D(num_channels*2, 1,  padding=same_padding,name='likelihood_params_conv')], name='output_group'))
