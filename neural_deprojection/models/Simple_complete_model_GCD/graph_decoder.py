...
class GraphMappingNetwork(AbstractModule):
    ...
    def _build(self, graphs):
        ...
        # tokens = H * W
        # batched_samples = S * batch
        # graphs.nodes [batched_samples * tokens, embedding_dim]
        # graphs.n_node = [tokens, tokens, ..., tokens], with shape [batched_samples]
        graphs = self.projection_node_block(graphs)  # [batched_samples * tokens, node_size]
        batched_graphs = graph_batch_reshape(graphs)  # nodes shape [batched_samples, tokens, node_size]
        ...
        concat_nodes = tf.concat([batched_graphs.nodes,
                                  tf.tile(self.empty_node_variable[None, :],
                                          [n_graphs, n_nodes_per_graph, 1])], axis=1)  # [batched_samples, 2 * tokens, node_size]
        ...
        concat_graphs = graph_unbatch_reshape(batched_graphs)  # nodes shape [batched_samples * 2 * tokens, node_size]
        ...
        def _core(latent_graphs, i):
            input_nodes = latent_graphs.nodes  # [batched_samples * 2 * tokens, node_size]
