...
def _create_autogressive_edges_from_nodes_dynamic(n_node, exclude_self_edges):
    ...
    receivers, senders = tf.transpose(tf.where(ind))
...
class GraphMappingNetwork(AbstractModule):
    ...
    def _build(self, graphs):
        ...
        """
        # give graphs' nodes a new node dimension (linear transformation)
        # n_node_per_graph = H * W
        # n_graphs = S * batch
        # graphs.nodes [n_graphs * n_node_per_graph, embedding_dim]
        # graphs.n_node = [n_node_per_graph, n_node_per_graph, ..., n_node_per_graph], with shape [n_graphs]
        graphs = self.projection_node_block(graphs)  # [n_graphs * n_node_per_graph, node_size]
        batched_graphs = graph_batch_reshape(graphs)  # nodes shape [n_graphs, n_node_per_graph, node_size]
        [n_graphs, n_nodes_per_graph, _] = get_shape(batched_graphs.nodes)
        concat_nodes = tf.concat([batched_graphs.nodes,
                                  tf.tile(self.empty_node_variable[None, :],
                                          [n_graphs, n_nodes_per_graph, 1])], axis=1)  # [n_graphs, 2 * n_node_per_graph, node_size]
        batched_graphs = batched_graphs.replace(nodes=concat_nodes,
                                                globals=tf.tile(self.starting_global_variable[None,:],[n_graphs, 1]),
                                                n_node=tf.constant(n_graphs * [2 * n_nodes_per_graph], dtype=tf.int32))
        concat_graphs = graph_unbatch_reshape(batched_graphs)  # nodes shape [n_graphs * 2 * n_node_per_graph, node_size]

        #nodes, senders, recievers, globals
        concat_graphs = autoregressive_connect_graph_dynamic(concat_graphs)  # exclude self edges because 3d n_node_per_graph orginally placeholder?

        latent_graphs = concat_graphs
        node_idx = -self.num_output

        def _core(latent_graphs, i):
            input_nodes = latent_graphs.nodes  # [n_graphs * 2 * n_node_per_graph, node_size]
            ...
            # ind = latent_graphs.receivers in [graph_offset + self.num_output + i for graph_offset in n_nodes_per_graph * tf.range(n_graphs)]
            ...
            #todo: mask
            latent_graphs = latent_graphs.replace(nodes=latent_graphs.nodes + input_nodes)  # residual connections   # why residual when 3d n_node_per_graph are placeholder orginally?
        ...
        return token_nodes  # [n_graphs, num_output, embedding_dim]