...
class DiscreteImageVAE(AbstractModule):
    def __init__(self,
        ...
        self.encoder = Encoder2D(hidden_size=hidden_size, num_embeddings=num_embedding, name='EncoderImage')
        self.decoder = Decoder2D(hidden_size=hidden_size, num_channels=num_channels, name='DecoderImage')
    ...
    def _build(self, img, **kwargs) -> dict:
        ...
        token_samples_onehot_perp = tf.reshape(token_samples_onehot, [self.num_token_samples, batch, H, W, self.num_embedding])

        q_dist = tfp.distributions.RelaxedOneHotCategorical(self.temperature, logits=latent_logits)
        log_prob_q = q_dist.log_prob(token_samples_onehot_perp)  # num_samples, batch, H, W
