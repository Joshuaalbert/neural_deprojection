...
from neural_deprojection.graph_net_utils import AbstractModule, get_shape, graph_batch_reshape, graph_unbatch_reshape, histogramdd, map_coordinates
...
class VoxelisedModel(AbstractModule):
    """
    img -> latent_2d (2D encoder pre-trained)
    latent_2d -> {latent_2d, latent_3d} (auto-regressive modelling next-token)
    latent_3d -> voxels_properties (3D autoregressive_prior)

    interpolate the voxels to the node positions to compute the loss.
    """
    def __init__(self,
                 num_properties: int,
                 voxel_per_dimension:int,
                 component_size:int,
                 num_embedding_3d:int,
                 edge_size:int,
                 global_size:int,
                 ...
                 num_token_samples: int,
                 multi_head_output_size: int,
                 decoder_3d_hidden_size: int,
                 num_heads: int,
                 batch: int,
                 name=None):
        super(VoxelisedModel, self).__init__(name=name)

        self.discrete_image_vae = discrete_image_vae
        num_components = voxel_per_dimension**3
        self.voxel_per_dimension = voxel_per_dimension
        
        self.autoregressive_prior = GraphMappingNetwork(num_output=num_components,
                                                        num_embedding=num_embedding_3d,
                                                        multi_head_output_size=multi_head_output_size,
                                                        num_heads=num_heads,
                                                        embedding_size=component_size,
        ...
        # output is [mu, log_stddev]
        self.decoder_3d = Decoder3D(decoder_3d_hidden_size, num_properties)
    ...
    def reconstruct_field(self, field_component_tokens, positions):
        ...
        latent_voxels = tf.reshape(field_component_tokens,
                            [self.num_token_samples * batch,
                             self.voxel_per_dimension, self.voxel_per_dimension, self.voxel_per_dimension,
                             self.component_size])
        ...
        def _interpolate_single_batch(args):
            ...
            arrays = [tf.linspace(pmin[0], pmax[0], N),
            ...
            values_at_positions = tf.vectorized_map(lambda voxels: map_coordinates(voxels, fractional_coordinates, order=1), voxels)
            # [n_node, num_properties*2]
            values_at_positions = tf.transpose(values_at_positions, (1,0))
            return values_at_positions
        values_at_positions = tf.vectorized_map(_interpolate_single_batch, (voxels, positions))
        values_at_positions = tf.reshape(values_at_positions, (self.num_token_samples, batch, n_node, 2*self.num_properties))
