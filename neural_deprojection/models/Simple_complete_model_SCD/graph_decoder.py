...
class GraphMappingNetwork(AbstractModule):
    ...
    Args:
        ...
        output_size: size of the nodes at output
    ...
    def __init__(self,
        ...
        self.starting_node_variable = tf.Variable(initial_value=tf.random.truncated_normal((num_output, node_size)),
        ...
        global_model_fn = lambda: snt.nets.MLP([global_size, global_size], activate_final=True,
    ...
    def _build(self, graphs):
        ...
        #n_graphs, n_node+num_output, node_size
        concat_nodes = tf.concat([batched_graphs.nodes, tf.tile(self.starting_node_variable[None, :], [n_graphs, 1, 1])], axis=0)
        ...
        def _core(output_token_idx, latent_graphs):
            ...
            batched_input_nodes = batched_latent_graphs.nodes#[n_graphs, n_node+num_output, node_size]
            latent_graphs = self.edge_block(latent_graphs)
            latent_graphs = self.node_block(latent_graphs)
            batched_latent_graphs = graph_batch_reshape(latent_graphs)
            token_3d_logits = batched_latent_graphs.nodes[:, -self.num_output, :] #n_graphs, num_output, num_embedding_3d
            token_distribution = tfp.distributions.RelaxedOneHotCategorical(temperature, logits=token_3d_logits)
            token_3d_samples_onehot = token_distribution.sample((1,),name='token_samples')  # [1, n_graphs, num_output, num_embedding_3d]
            token_3d_samples_onehot = token_3d_samples_onehot[0] # [n_graphs, num_output, num_embedding_3d]
            token_3d_samples = tf.einsum('goe,ed->god',token_3d_samples_onehot, self.embeddings) # n_graphs,num_ouput, embedding_dim
            mask = tf.range(self.num_output) == output_token_idx#num_output
            mask = tf.concat([tf.zeros(n_node_per_graph_before_concat, dtype=tf.bool), mask])#n_node+num_output
            mask = tf.tile(mask,[n_graphs])#
            #n_graphs, n_node+num_output, node_size
            output_nodes = tf.where(mask[None,:, None],
                                    tf.concat([tf.zeros([n_graphs, n_node_per_graph_before_concat, self.node_size]), token_3d_samples], axis=1),
                                    batched_input_nodes)
        ...
        _, latent_graphs = tf.while_loop(cond=lambda output_token_idx, state: output_token_idx < self.num_output,
                                        body=lambda output_token_idx, state: _core(output_token_idx + 1, state),
