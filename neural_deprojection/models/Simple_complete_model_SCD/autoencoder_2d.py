...
class DiscreteImageVAE(AbstractModule):
    def __init__(self,
        ...
        self.encoder = EncoderIm(hidden_size=hidden_size, num_embeddings=num_embedding, name='EncoderImage')
        self.decoder = DecoderIm(hidden_size=hidden_size, num_channels=num_channels, name='DecoderImage')
    ...
    def _build(self, img, **kwargs) -> dict:
        ...
        temperature = tf.maximum(0.1, tf.cast(1. - 0.1/(self.step/1000), dtype=tf.float32))
        token_distribution = tfp.distributions.RelaxedOneHotCategorical(temperature, logits=logits)
        ...
        if self.step % 1 == 0:
            img_mu_0 = decoded_ims[0][..., :self.num_channels]
