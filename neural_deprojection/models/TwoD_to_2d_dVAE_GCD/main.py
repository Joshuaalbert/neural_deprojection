...
def build_dataset(tfrecords, batch_size):
    ...
    dataset = dataset.map(lambda graph_data_dict, img, cluster_idx, projection_idx, vprime:
                          img).shuffle(buffer_size=50)  # .batch(batch_size=batch_size)
...
def train_disc_img_vae(data_dir, config, kwargs):
    ...
    train_dataset = build_dataset(train_tfrecords, batch_size=2)
    test_dataset = build_dataset(test_tfrecords, batch_size=2)
...
def main(data_dir):
    ...
    kwargs = dict(num_token_samples=1)
