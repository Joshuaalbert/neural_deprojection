...
def render_image(snap, qty='rho', x2=100, nx=500, y2=None, ny=None, x1=None,
                 ...
                 threaded=None,
                 denoise=None, magnification=1.0, offset = [0.,0.,0,]):
    """
    ...
    *z_camera*: If this is set, a perspective image is rendered,
    ...
    if threaded:
        im = _threaded_render_image(base_renderer, snap, qty, x2, nx, y2, ny, x1, y1, z_plane,
                                    ...
                                    num_threads=threaded, magnification=magnification, offset = offset)
    else:
        im = base_renderer(snap, qty, x2, nx, y2, ny, x1, y1, z_plane,
                           out_units, xy_units, kernel, z_camera, smooth,
                           smooth_in_pixels, False, magnification=magnification, offset = offset)
...
def _render_image(snap, qty, x2, nx, y2, ny, x1,
                  ...
                  __threaded=False, magnification=1.0, offset = [0.,0.,0,]):
    ...
    #Added for centralization
    x = magnification*(x - (np.max(x) + np.min(x)) / 2) + offset[0]
    y = magnification*(y - (np.max(y) + np.min(y)) / 2) + offset[1]
    z = z - (np.max(z) + np.min(z)) / 2 + offset[2]
