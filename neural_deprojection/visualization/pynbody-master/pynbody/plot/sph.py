...
def velocity_image(sim, width="10 kpc", vector_color='black', edgecolor='black', quiverkey_bg_color=None,
                   ...
                   vector_qty='vel', filename=None, magnification = 1.0, offset = [0.,0.,0.], **kwargs):
    """
    ...
    *vector_qty* ('vel'): The name of the vector field to plot
    ...
    vx = image(sim, qty=vx_name, width=width, log=False,
               resolution=vector_resolution, noplot=True,av_z=av_z, magnification=magnification, offset=offset)
    ...
    im = image(sim, width=width, **kwargs, magnification=magnification, offset=offset)
    ...
    if mode == 'quiver':
        ...
        if quiverkey:
        	qk = p.quiverkey(Q, key_x, key_y, key_unit.in_units(sim['vel'].units, **sim.conversion_context()),
                    r"$\mathbf{" + key_unit.latex() + "}$", labelcolor=key_color, color=key_color, fontproperties={'size': 12})
...
def volume(sim, qty='rho', width=None, resolution=200,
    ...
    *create_figure* (True): create a new mayavi figure before rendering
    ...
    grid_data = sph.to_3d_grid(sim,qty=qty,nx=resolution,
                               x2=None if width is None else width/2, threaded=False)
    print(np.max(grid_data))
    print(np.min(grid_data))
...
def image(sim, qty='rho', width="10 kpc", resolution=500, units=None, log=True,
          ...
          noplot=False, ret_im=False, fill_nan=True, fill_val=0.0, linthresh=None, magnification=1.0,
          offset = [0.,0.,0.], **kwargs):
    """
    ...
    *linthresh* (None): if the image has negative and positive values
    ...
    if av_z:
        if isinstance(kernel, sph.Kernel2D):
            ...
            im = sph.render_image(sim, qty, width/ 2, resolution, out_units=aunits, kernel=kernel,
                                              z_camera=z_camera, **kwargs, threaded=False, magnification=magnification, offset=offset)
            im2 = sph.render_image(sim, av_z, width / 2, resolution, out_units=aunits, kernel=kernel,
                                  z_camera=z_camera, **kwargs, threaded=False, magnification=magnification, offset=offset)
            # im = sph.render_image(sim, qty, x2 = -210000, x1 = -230000, nx=resolution, y1=-190000, y2=-170000, out_units=aunits, kernel=kernel,
            #             #                       z_camera=z_camera, **kwargs)
            #             # im2 = sph.render_image(sim, av_z, x2 = -210000, x1 = -230000, nx=resolution,y1=-190000, y2=-170000, kernel=kernel,
            #             #                        z_camera=z_camera, **kwargs)
        ...
        im = sph.render_image(sim, qty, width / 2, resolution, kernel=kernel,
                              z_camera=z_camera, **kwargs, magnification=magnification, offset=offset, threaded=False)
    ...
    if not noplot:
        ...
        if ret_im:
            return p.imshow(im[::-1, :].view(np.ndarray), extent=(-width / 2 / magnification, width / 2 / magnification,
                                                                  -width / 2 / magnification, width / 2 / magnification),
                            vmin=vmin, vmax=vmax, cmap=cmap, norm = norm)

        ims = p.imshow(im[::-1, :].view(np.ndarray), extent=(-width / 2 / magnification, width / 2 / magnification,
                                                             -width / 2 / magnification, width / 2 / magnification),
                       vmin=vmin, vmax=vmax, cmap=cmap, norm = norm)
